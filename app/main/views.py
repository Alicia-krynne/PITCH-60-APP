...
def index():
    ...
    bonvoyage = Pitch.query.filter_by(category = 'Bonvoyge').all() 
    concert = Pitch.query.filter_by(category = 'Concerts').all()
    run = Pitch.query.filter_by(category = 'Run').all()
    return render_template('index.html', pitches = pitches,bonvoyage = bonvoyage, concert= concert,run=run)
